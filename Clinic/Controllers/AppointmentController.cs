...
namespace Clinic.Controllers
{
    ...
    public class AppointmentController : BaseController
    {
                ...
                if (string.IsNullOrEmpty(currentUserRole) || currentUserRole == "Campus Clinic" || currentUserRole == "2")
                {
                    var allQuery = from a in _context.Appointments
                                   ...
                                   select new
                                   {
                                       ...
                                       className = a.Status == AppointmentStatus.Pending ? "pending-appointment" : "",
                                       extendedProps = new
                                       {
                                           userId = a.UserId,
                                           doctorId = a.DoctorId,
        ...
        [HttpPost("Reject/{id}")]
        public async Task<JsonResult> RejectAppointment(int id)
        {
            try
            {
                var appointment = await _context.Appointments.FirstOrDefaultAsync(a => a.Id == id);
                ...
                if (appointment.Status != AppointmentStatus.Pending)
                {
                    return Json(new { success = false, message = "Only pending appointments can be rejected." });
                }

                appointment.Status = AppointmentStatus.Rejected;
                await _context.SaveChangesAsync();

                return Json(new { success = true, message = "Appointment rejected successfully!" });
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Error rejecting appointment: {ex.Message}");
                return Json(new { success = false, message = "An error occurred while rejecting the appointment." });
            }
        }

        [HttpPost("Complete/{id}")]
        public async Task<JsonResult> CompleteAppointment(int id)
        {
            try
            {
                var appointment = await _context.Appointments.FirstOrDefaultAsync(a => a.Id == id);
                ...
                {
                    return Json(new { success = false, message = "Only approved appointments can be finished." });
                }

                appointment.Status = AppointmentStatus.Finished;
                await _context.SaveChangesAsync();

                return Json(new { success = true, message = "Appointment marked as finished!" });
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Error completing appointment: {ex.Message}");
                return Json(new { success = false, message = "An error occurred while finishing the appointment." });
            }
        }
        ...
        public async Task<JsonResult> CancelAppointment(int id)
        {
            try
            {
                ...
                {
                    return Json(new { success = false, message = "Cannot cancel a finished or already cancelled appointment." });
                }
        ...
        public async Task<JsonResult> GetAvailableTimeSlots(int? doctorId = null, DateTime? date = null)
        {
            try
            {
                var query = _context.TimeSlots
                    .Include(ts => ts.Doctor)
                    .Where(ts => ts.AppointmentId == null);
        ...
        private async Task PopulateViewBags()
        {
            ...
            ViewBag.Doctors = await _context.Users
                .Where(u => u.Role == 1)
                .Select(u => new { u.Id, FullName = $"{u.FirstName} {u.LastName}", Specialization = u.Program })
                .ToListAsync();
        }
